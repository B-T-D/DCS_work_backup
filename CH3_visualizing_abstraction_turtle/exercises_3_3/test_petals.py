...
class TestNumSegments(unittest.TestCase):
    ...
    def test_num_segments_returns_int(self):
        """Confirms that the function that returns the number of segments
        returns an integer."""
...
class TestDegrees(unittest.TestCase):
    """Tests for the function(s) that return the number of degrees to be used
    in turning the turtle 'arrow' to draw the next segment."""

    def test_multiple_of_360(self):
        """Is total number of degrees (degrees * number of petals) a multiple
        of 360?"""
        for i in [5, 7, 8, 10, 11, 12, 13]:
            print(i)
