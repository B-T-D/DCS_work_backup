...
def get_word_counts(text, word, window_length):
    ...
    Postcondition: Returns two objects, one float giving the average count per
        window, and one list, listing the counts for all windows.
    ...
    for index in range(0, len(text) - window_length + 1, window_length):
        ...
        word_counts.append(words_in_window)

    # double check against a division by zero error:
    assert window_count > 0, 'no windows were found'
    return word_count / window_count, word_counts
    # This returns a tuple object
