...
def reduce2(photo):
    """Reduces an image by the same amount by instead replacing each 2 x 2 block with
    a pixel that has the average color of the pixels in block."""
    pass
    # schedule hard stop

def negative(photo):
    """Returns an image object that is the negative of the original."""
    width = photo.width()
    height = photo.height()
    new_photo = image.Image(width, height, title='Negative image')
    for y in range(height):
        for x in range(width):
            color = photo.get(x, y)
            new_photo.set(x, y, negate_pixel(color))
    return new_photo
    
def negate_pixel(color):
    """Returns new tuple equal to 255 - element for each element in (tuple)
    color."""
